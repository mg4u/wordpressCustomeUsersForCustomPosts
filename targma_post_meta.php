...
add_action( 'add_meta_boxes', 'targma_featured_meta' );
function targma_featured_meta() {
    global $post;
    // var_dump($post);
    add_meta_box( 'phoneNumber', 'رقم الجوال', 'targma_text_form', 'targma', 'normal', 'low' );
    add_meta_box( 'email', 'البريد الالكتروني', 'targma_text_form', 'targma', 'normal', 'low' );
    add_meta_box( 'gpa', 'المعدل التراكمي', 'targma_text_form', 'targma', 'normal', 'low' );

    add_meta_box( 'student', ' الطالبة ', 'targma_student_form', 'targma', 'normal', 'low' );
    add_meta_box( 'warning', ' التحقق من الانذارات ', 'targma_warning_form', 'targma', 'normal', 'low' );
    
    add_meta_box( 'schoolSchedule', ' صورة الجدول الدراسي المسجل لك هذا الفصل ', 'targma_attach_form', 'targma', 'normal', 'low' );
    add_meta_box( 'academicRecord', ' صورة من السجل الأكاديمي كاملا ', 'targma_attach_form', 'targma', 'normal', 'low' );
    
    add_meta_box( 'academicNotes', 'اضافة ملاحظات على الوضع الاكاديمي', 'targma_text_area_form', 'targma', 'normal', 'low' );
    add_meta_box( 'scheduleTime', 'اقصى موعد لاستلام الجدول', 'targma_text_form', 'targma', 'normal', 'low' );

    add_meta_box( 'requestStatus', ' الاجراءات المتخذه للحاله من قبل الموظفة ', 'targma_requestStatus_form', 'targma', 'normal', 'low' );
}

add_action( 'save_post', 'save_targma_featured_meta' );

function save_targma_featured_meta( $post_ID ) {
    global $post;
    if(in_array($post->post_type,['targma','targma','targma'])) {
        if (isset( $_POST ) ) {
...
function targma_text_form( $post,$args ) {
    $fieldName=$args['id'];
...
function targma_text_area_form( $post,$args ) {
	$fieldName=$args['id'];
    $fieldValue = get_post_meta($post->ID,$fieldName,true);
    ?>
...
function targma_student_form( $post ) {
	$args = array(
	    'role'    => 'student',
...
function targma_warning_form( $post,$args ) {
	$fieldName=$args['id'];
...
function targma_requestStatus_form( $post,$args ) {
...
function targma_attach_form( $post,$args ) {
